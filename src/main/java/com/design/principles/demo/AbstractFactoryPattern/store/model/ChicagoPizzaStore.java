...
public class ChicagoPizzaStore implements PizzaStore{
    ...
    public Pizza getPizza(String type) {
        ...
        if(type.equalsIgnoreCase("clam")) {
            Pizza pizza = new SimplePizza(new ThickCrustDough(), new TomatoSauce(), List.of(new Capsicum(), new Tomato(), new Onion(), new Garlic()), new ReggianoCheese(), new SlicedPepperoni(), new FreshClam());
            pizza.setName("Chicago Clam Pizza");
            preparePizza(pizza);
            return pizza;
        } else if(type.equalsIgnoreCase("veggie")) {
            Pizza pizza = new SimplePizza(new ThickCrustDough(), new MarinaraSauce(), List.of(new Capsicum(), new Tomato(), new Onion(), new Garlic(), new Mushroom()), new MarrianoCheese(), new SlicedPepperoni(), new UsedClam());
            pizza.setName("Chicago Veggie Pizza");
            preparePizza(pizza);
            return pizza;
        } else {
            Exception ex = new RuntimeException("Not compatible with any Chicago Pizza type");
            throw ex;
        }
        } catch(Exception ex) {
            System.out.println("Eception while mapping Chicago pizza");
        }
